...
def getSeries():
    sql_query = 'SELECT SeriesId, SeriesTitle FROM [dbo].[Series]'
    rows = getAll(sql_query=sql_query)
    
    return rows

def getSeasons():
    sql_query = ''' SELECT SeasonId, SeasonTitle, SeriesTitle
                     FROM [dbo].[Seasons] INNER JOIN [dbo].[Series]
                     ON ([dbo].[Seasons].SeriesId = [dbo].[Series].SeriesId)'''
    rows = getAll(sql_query=sql_query)
...
def getVideos():
    ...
    sql_query = 'SELECT VideoYouTubeId, SeasonAppearanceId FROM [dbo].[Videos]'
    rows = getAll(sql_query=sql_query)
...
def insert(sql_query, params):
    try:
        ...
        # TODO: @@IDENTITY gives the last inserted id of any scope in the connection.
        # I would like to usue SCOPE_IDENTITY() instead but was having trouble
        # getting that functional with pyodbc.
        sql_query = 'SELECT @@IDENTITY'
        id = cursor.execute(sql_query).fetchval()
        conn.commit()
...
def insertSeries(series_title):
    ...
    sql_query = '''
        ...
        VALUES (?)
    '''
    series_id = insert(sql_query=sql_query, params=(series_title))
...
def insertSeason(season):
    ...
    season_id = insert(sql_query=sql_query, params=params)
...
def insertChannel(channel):
    ...
    channel_internal_id = insert(sql_query=sql_query, params=params)
...
def insertSeasonAppearance(season_appearance):
    ...
    season_appearance_internal_id = insert(sql_query=sql_query, params=params)
...
def insertVideo(video):
    ...
    video_internal_id = insert(sql_query=sql_query, params=params)
